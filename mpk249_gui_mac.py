...
os.environ["CTK_FORCE_WIDGETS"] = "1"

def resource_path(relative_path):
    """ Get absolute path to resource, works for dev and for PyInstaller """
    try:
        # PyInstaller creates a temp folder and stores path in _MEIPASS
        # Per le app in formato cartella, il percorso è diverso
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.knob_buttons, self.fader_widgets, self.switch_buttons, self.pad_buttons, self.daw_buttons = [], [], [], {}, {}
        self.control_bank_buttons, self.pad_bank_buttons = {}, {}
        self.ACTIVE_COLOR, self.INACTIVE_COLOR = "#337AB7", "gray40"

        self.GROUP_COORDINATES = {
            'knobs': {'rx': 0.648000, 'ry': 0.235423}, 'faders': {'rx': 0.658000, 'ry': 0.322250},
            'switches': {'rx': 0.654800, 'ry': 0.467689}, 'daw': {'rx': 0.159940, 'ry': 0.313869},
            'pad_banks': {'rx': 0.238000, 'ry': 0.357613}, 'pads': {'rx': 0.2801000, 'ry': 0.245200},
            'control_bank': {'rx': 0.507000, 'ry': 0.470000},
        }

        # --- LAYOUT A GRIGLIA, MA PIÙ ROBUSTO ---
        # Configura la griglia della finestra principale
        self.grid_columnconfigure(0, weight=1) # La colonna 0 (l'unica) si espande
        self.grid_rowconfigure(2, weight=1)    # La riga 2 (quella del main_frame) si espande

        # --- Frame superiori posizionati con .grid() ---
        ...
        self.main_frame.grid(row=2, column=0, sticky="nsew", padx=10, pady=(0, 10))

        try:
            image_path = resource_path("keyboard.jpg")
            self.original_image = Image.open(image_path)
            self.IMAGE_WIDTH, self.IMAGE_HEIGHT = self.original_image.size
            
            # Non impostiamo l'altezza della finestra totale, lasciamo che la griglia faccia il suo lavoro
            # ma impostiamo una dimensione minima per garantire che il main_frame non collassi
            min_height = self.IMAGE_HEIGHT + 150
            self.geometry(f"{self.IMAGE_WIDTH}x{min_height}")
            self.minsize(self.IMAGE_WIDTH, min_height) # Imposta la dimensione minima
            self.resizable(False, False)

            # Forza l'aggiornamento PRIMA di qualsiasi cosa
            self.update_idletasks()
            
            self.place_background_image()
            self.place_static_hotspots()
            ...
            self.after(100, force_redraw) # Esegui dopo 100ms
            
        except FileNotFoundError:
            self.geometry("600x200") # Finestra di errore leggibile
            error_message = "Errore: 'keyboard.jpg' non trovato."
            ctk.CTkLabel(self.main_frame, text=error_message, font=("Arial", 20), text_color="orange").pack(fill="both", expand=True)

        self.protocol("WM_DELETE_WINDOW", self.on_closing)


    def place_background_image(self):
        # VERSIONE CORRETTA: non usa più offset o WINDOW_WIDTH
        self.bg_image_ctk = ctk.CTkImage(light_image=self.original_image, size=(self.IMAGE_WIDTH, self.IMAGE_HEIGHT))
        self.bg_label = ctk.CTkLabel(self.main_frame, text="", image=self.bg_image_ctk)
        # Posiziona l'immagine all'angolo in alto a sinistra del suo frame contenitore
        self.bg_label.place(x=0, y=0)

    def place_static_hotspots(self):
        # VERSIONE CORRETTA: non usa più offset_x o offset_y
        def create_hotspot(group_name, rel_x, rel_y, w, h, command):
            group_coords = self.GROUP_COORDINATES[group_name]
            # Le coordinate ora sono calcolate direttamente rispetto all'angolo del frame (0,0)
            base_x = group_coords['rx'] * self.IMAGE_WIDTH
            base_y = group_coords['ry'] * self.IMAGE_HEIGHT
            ...
            button.place(x=base_x + rel_x, y=base_y + rel_y)
            return button

        # Il resto di questa funzione (i cicli for) rimane identico
        ...
        base_x = group_coords_faders['rx'] * self.IMAGE_WIDTH
        base_y = group_coords_faders['ry'] * self.IMAGE_HEIGHT
        for i in range(8):
            x_pos = base_x + i * 38.7
        ...
        base_x_pad_banks = group_coords_pad['rx'] * self.IMAGE_WIDTH
        base_y_pad_banks = group_coords_pad['ry'] * self.IMAGE_HEIGHT
        for i, label in enumerate(["A", "B", "C", "D"]):
            ...
            btn.place(x=base_x_pad_banks, y=base_y_pad_banks + i * 23)
            self.pad_bank_buttons[label] = btn

        group_coords_ctrl = self.GROUP_COORDINATES['control_bank']
        base_x_ctrl = group_coords_ctrl['rx'] * self.IMAGE_WIDTH
        base_y_ctrl = group_coords_ctrl['ry'] * self.IMAGE_HEIGHT
        for i, label in enumerate(["A", "B", "C"]):
            ...
            btn.place(x=base_x_ctrl + (i * 28), y=base_y_ctrl)
            self.control_bank_buttons[label] = btn
        ...
        try:
            while self.is_listening:
                ...
                if msg:
                    ...
                    if not is_assembling and 0xF0 in chunk:
                        start_index = chunk.index(0xF0)
                        sysex_buffer.extend(chunk[start_index:])
                        is_assembling = True
                    elif is_assembling:
                        sysex_buffer.extend(chunk)

                    if is_assembling and 0xF7 in sysex_buffer:
                        end_index = sysex_buffer.index(0xF7)
                        complete_sysex = bytes(sysex_buffer[:end_index + 1])
                        
                        # Controlla che sia un preset valido Akai (lungo almeno 500 byte)
                        if len(complete_sysex) > 500:
                            print(f"Messaggio SysEx completo ricevuto ({len(complete_sysex)} bytes).")
                            self.is_listening = False  # Abbiamo finito, smetti di ascoltare
                            self.after(0, self.load_preset_from_data, complete_sysex)
                        else:
                            print(f"Ricevuto messaggio SysEx corto ({len(complete_sysex)} bytes), lo ignoro.")
                            sysex_buffer = bytearray()
                            is_assembling = False
                
                time.sleep(0.001)
        finally:
            # QUESTO BLOCCO VIENE ESEGUITO SEMPRE, sia che il thread finisca
            # normalmente, sia che venga interrotto da stop_midi_listener.
            # È il posto giusto per chiudere la porta.
            print("Listener thread terminato, chiusura porta MIDI IN.")
            midi_in.close_port()
